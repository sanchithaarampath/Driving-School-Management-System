...
export class BillingList implements OnInit {
  ...
  downloadReceipt(billId: number, billNumber: string) {
    this.http.get(`${this.apiUrl}/billing/${billId}/receipt/pdf`, {
      ...
      error: () => {
        // Fallback: open HTML receipt in a new tab if PDF endpoint not available
        this.http.get(`${this.apiUrl}/billing/${billId}/receipt`, {
          headers: this.getHeaders(), responseType: 'text'
        }).subscribe({ next: (html) => { const w = window.open('', '_blank'); if (w) { w.document.write(html); w.document.close(); } } });
      }
    });
  }

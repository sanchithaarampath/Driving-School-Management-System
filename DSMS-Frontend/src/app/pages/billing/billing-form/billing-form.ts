...
export class BillingForm implements OnInit {
  ...
  packageName          = '';
  packagePrice         = 0;
  packageMaxDiscount   = 0;
  previouslyPaid       = 0;
  isLoadingBalance     = false;

  get remainingBalance()      { return Math.max(0, this.packagePrice - this.previouslyPaid); }
  get balanceAfterPayment()   { return Math.max(0, this.remainingBalance - this.discountAmount - this.instalmentAmount); }
  ...
  selectStudent(s: any) {
    ...
    this.http.get<any>(`${this.apiUrl}/student/${s.id}`, { headers: this.getHeaders() }).subscribe({
      next: (profile) => {
        ...
        this.packageMaxDiscount = profile.coursePackage?.maxDiscount ?? 0;
  ...
  onSubmit() {
    ...
    if (this.discountAmount < 0) { this.errorMessage = 'Discount cannot be negative.'; return; }
    if (this.packageMaxDiscount > 0 && this.discountAmount > this.packageMaxDiscount) {
      this.errorMessage = `Discount exceeds the package maximum of Rs. ${this.packageMaxDiscount.toLocaleString()}.`;
      return;
    }
    if (this.discountAmount > this.remainingBalance) {
      this.errorMessage = 'Discount cannot exceed the remaining balance.';
      return;
    }
    if (this.instalmentAmount > (this.remainingBalance - this.discountAmount)) {
      this.errorMessage = `Amount exceeds balance after discount of Rs. ${(this.remainingBalance - this.discountAmount).toLocaleString()}.`;
      return;
    }

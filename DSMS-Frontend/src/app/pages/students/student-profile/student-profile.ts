...
export class StudentProfile implements OnInit {
  ...
  errorMessage = '';
  billsError = '';
  activeTab: 'info' | 'payments' | 'docs' | 'training' = 'info';
  ...
  // ── Training Sessions ─────────────────────────────────────────────────────
  loadTrainingSessions(studentId: number) {
    this.trainingLoading = true;
    this.http.get<any[]>(`${this.apiUrl}/training-sessions/student/${studentId}`, { headers: this.getHeaders() }).subscribe({
      next: (data) => { this.trainingSessions = data; this.trainingLoading = false; },
      error: () => { this.trainingLoading = false; }
    });
    this.http.get<any>(`${this.apiUrl}/training-sessions/summary/student/${studentId}`, { headers: this.getHeaders() }).subscribe({
      next: (data) => { this.trainingSummary = data; },
      error: () => {}
    });
  }

  switchTab(tab: 'info' | 'payments' | 'docs' | 'training') {
    this.activeTab = tab;
    if (tab === 'training' && this.trainingSessions.length === 0) {
      this.loadTrainingSessions(this.studentId);
    }
  }

  bookTrainingSession() {
    this.router.navigate(['/training-schedule/new'], { queryParams: { studentId: this.studentId } });
  }

  trainingStatusClass(status: string) {
    switch (status) {
  ...
  downloadReceipt(billId: number, billNumber: string) {
    this.http.get(`${this.apiUrl}/billing/${billId}/receipt/pdf`, {
      ...
      next: (blob) => {
        ...
        a.href    = url;
        a.download = `${billNumber || 'receipt'}.pdf`;
        a.click();
        URL.revokeObjectURL(url);
      },
      error: () => {
        // Fallback: open HTML receipt with auto-print if PDF endpoint not available
        this.http.get(`${this.apiUrl}/billing/${billId}/receipt`, {
          headers: this.getHeaders(), responseType: 'text'
        }).subscribe({
          next: (html) => {

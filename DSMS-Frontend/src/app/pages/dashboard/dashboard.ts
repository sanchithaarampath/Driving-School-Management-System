...
export class Dashboard implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.authService.isCompanyAdmin()) {
      const saved = localStorage.getItem('dashboard_branchId');
      if (saved) this.selectedBranchId = +saved || 0;
    }
    this.loadDashboard();
  ...
  onBranchFilterChange(val: any) {
    ...
    // Persist choice so it survives page navigation
    localStorage.setItem('dashboard_branchId', this.selectedBranchId.toString());
    this.loadDashboard();
  }

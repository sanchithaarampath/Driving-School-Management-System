...
namespace DSMS.API.Controllers
{
    ...
    public class StudentController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            // Instructors see only students they have training sessions with (cross-branch)
            if (role == "Instructor")
            {
                ...
                var studentIds = await _context.TrainingSessions
                    .Where(t => t.InstructorId == instructor.Id)
                    .Select(t => t.StudentId)
                    .Distinct()
                    .ToListAsync();

                var instructorStudents = await _context.Students
                    .Include(s => s.Branch)
                    .Where(s => studentIds.Contains(s.Id) && s.Active == true)
                    .Select(s => new {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            return Ok(new {
                ...
                student.PostalCode, student.ExistingLicenseNo,
                student.IssuedLicenseNo, student.LicenseIssuedDate,
                student.PackageType,
        ...
        [HttpPost]
        [Authorize(Roles = "Company Admin,Admin,Branch Admin,Staff,OfficeStaff")]
        public async Task<IActionResult> Create([FromBody] StudentCreateExtDto dto)
        {
            var callerBranchId = ClaimsHelper.GetBranchId(User);
            var branchId = callerBranchId ?? dto.BranchId;

            // Validate branchId exists — if caller's JWT branchId is stale, fall back to dto
            if (branchId > 0 && !await _context.Branches.AnyAsync(b => b.Id == branchId))
                branchId = dto.BranchId;

            if (branchId <= 0 || !await _context.Branches.AnyAsync(b => b.Id == branchId))
                return BadRequest(new { message = "Invalid or missing branch. Please select a valid branch." });
        ...
        // PATCH /api/student/{id}/license — record issued license after passing exam
        [HttpPatch("{id}/license")]
        [Authorize(Roles = "Company Admin,Admin,Branch Admin,Staff")]
        public async Task<IActionResult> UpdateLicense(int id, [FromBody] LicenseUpdateDto dto)
        {
            var student = await _context.Students.FindAsync(id);
            if (student == null) return NotFound(new { message = "Student not found." });

            var callerBranchId = ClaimsHelper.GetBranchId(User);
            if (callerBranchId.HasValue && student.BranchId != callerBranchId) return Forbid();

            student.IssuedLicenseNo      = dto.IssuedLicenseNo;
            student.LicenseIssuedDate    = string.IsNullOrEmpty(dto.LicenseIssuedDate)
                                           ? null
                                           : DateTime.Parse(dto.LicenseIssuedDate);
            student.LastModifiedBy       = User.Identity?.Name ?? "system";
            student.LastModifiedDateTime = DateTime.Now;

            await _context.SaveChangesAsync();
            return Ok(new { message = "License details saved." });
        }
        ...
        public async Task<IActionResult> Search([FromQuery] string? q, [FromQuery] string? name, [FromQuery] string? nic, [FromQuery] string? phone)
        {
            ...
            // Instructors can only search within their own students (from training sessions)
            if (role == "Instructor")
            {
                var userSecurityId = int.TryParse(User.FindFirst("UserSecurityId")?.Value, out var uid) ? uid : 0;
                var instructor     = await _context.Instructors.FirstOrDefaultAsync(i => i.UserId == userSecurityId);
                if (instructor != null)
                {
                    var studentIds = await _context.TrainingSessions
                        .Where(t => t.InstructorId == instructor.Id)
                        .Select(t => t.StudentId).Distinct().ToListAsync();

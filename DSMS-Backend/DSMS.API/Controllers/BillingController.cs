...
namespace DSMS.API.Controllers
{
    ...
    public class BillingController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] BillCreateDto dto)
        {
            ...
            // Sum of all previous instalments for this student (only active bills)
            var previouslyPaid = await _context.Bills
                .Where(b => b.StudentId == dto.StudentId && b.Active != false)
                .SumAsync(b => (decimal?)b.PaidAmount) ?? 0;

            var effectiveRemaining = dto.PackagePrice - previouslyPaid - dto.DiscountAmount;
            if (dto.DiscountAmount < 0)
                return BadRequest(new { message = "Discount cannot be negative." });
            if (dto.DiscountAmount > (dto.PackagePrice - previouslyPaid))
                return BadRequest(new { message = "Discount exceeds the remaining balance." });
            if (dto.InstallmentAmount > effectiveRemaining)
                return BadRequest(new { message = "Instalment exceeds the remaining balance after discount." });

            var balanceAfter = Math.Max(0, dto.PackagePrice - previouslyPaid - dto.DiscountAmount - dto.InstallmentAmount);
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, new { message = $"Failed to save instalment: {inner}" });
            }
        }
        ...
        // ── Overpaid Analysis — Company Admin only ───────────────────────────────
        [HttpGet("overpaid-analysis")]
        public async Task<IActionResult> GetOverpaidAnalysis()
        {
            var role = User.FindFirst(System.Security.Claims.ClaimTypes.Role)?.Value ?? "";
            if (role != "Company Admin" && role != "Admin")
                return Forbid();

            // Get all active students with packages
            ...
            {
                var packagePrice = student.CoursePackage?.Price ?? 0;
                if (packagePrice <= 0) continue;

                var bills = await _context.Bills
                    .Where(b => b.StudentId == student.Id && b.Active != false)
                    .OrderByDescending(b => b.BillDate)
                    .Select(b => new { b.Id, b.BillNumber, b.BillDate, b.PaidAmount, b.BalanceAmount })
                    .ToListAsync();
                ...
                if (totalPaid > packagePrice)
                {
                    result.Add(new
                    {
                        ...
                        packagePrice,
                        totalPaid,
                        overpaidBy   = totalPaid - packagePrice,
        ...
        // ── Delete all bills for a student and let them start fresh ─────────────
        [HttpDelete("student/{studentId}/all-bills")]
        public async Task<IActionResult> DeleteAllBillsForStudent(int studentId)
        {
            var role = User.FindFirst(System.Security.Claims.ClaimTypes.Role)?.Value ?? "";
            if (role != "Company Admin" && role != "Admin")
                return Forbid();
            ...
            if (!bills.Any())
                return NotFound(new { message = "No bills found for this student." });

            // Hard-delete payments, soft-delete bills
            foreach (var bill in bills)
            {
                if (bill.Payments?.Any() == true)
                    _context.Payments.RemoveRange(bill.Payments);
            ...
            await _context.SaveChangesAsync();
            return Ok(new { message = $"Cleared {bills.Count} bill(s) for student ID {studentId}. They can now start fresh." });
        }

        // ── Audit Log — basic, from existing bill metadata ───────────────────────
        [HttpGet("audit-log")]
        public async Task<IActionResult> GetAuditLog()
        {
            var role = User.FindFirst(System.Security.Claims.ClaimTypes.Role)?.Value ?? "";
            if (role != "Company Admin" && role != "Admin" && role != "Branch Admin")
                return Forbid();
            ...
            var entries = await query
                ...
                .Select(b => new
                {
                    ...
                    studentName = b.Student.StudentName,
                    action      = b.Active == false ? "Deleted" : "Created",
                    amount      = b.PaidAmount,
                    status      = b.Status,
                    performedBy = b.Active == false ? (b.LastModifiedBy ?? "—") : (b.CreatedBy ?? "—"),
                    performedAt = b.Active == false ? b.LastModifiedDateTime : b.CreatedDateTime,
                    isDeleted   = b.Active == false
                })
                .ToListAsync();
        ...
        // GET /api/billing/{id}/receipt/pdf — download professional PDF receipt
        ...
        public async Task<IActionResult> GetReceiptPdf(int id)
        {
            ...
            var pdfBytes = _receiptService.GenerateReceiptPdf(receiptData);

...
namespace DSMS.API.Controllers
{
    ...
    public class UserManagementController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] CreateUserDto dto)
        {
            ...
            // Auto-create Instructor profile when role is Instructor
            var createdRole = await _context.Roles.FindAsync(dto.RoleId);
            if (createdRole?.RoleName == "Instructor" && dto.BranchId.HasValue)
            {
                var alreadyExists = await _context.Instructors.AnyAsync(i => i.UserId == userSec.Id);
                if (!alreadyExists)
                {
                    _context.Instructors.Add(new Instructor
                    {
                        InstructorName  = dto.UserFullName,
                        Nic             = "N/A",
                        Phone           = "N/A",

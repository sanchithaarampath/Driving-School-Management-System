...
public class ReceiptService : IReceiptService
{
    ...
    {
        _config = config;
        // QuestPDF community licence (free for non-commercial / academic projects)
...
<div class='receipt'>
  ...
  <div class='footer'>
    ...
    // ── QuestPDF — professional PDF receipt ───────────────────────────────────
    public byte[] GenerateReceiptPdf(ReceiptData data)
    {
        ...
        // Colour palette
        var red        = Colors.Red.Medium;
        var darkText   = Colors.Grey.Darken4;
        var mutedText  = Colors.Grey.Medium;
        var greenText  = Colors.Green.Medium;
        var yellowText = Colors.Orange.Medium;
        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                // ── Header ────────────────────────────────────────────────
                page.Header().Background(red).Padding(12).Column(col =>
                {
                    col.Item().AlignCenter().Text("🚗 " + schoolName)
                        .Bold().FontSize(14).FontColor(Colors.White);
                    if (!string.IsNullOrEmpty(schoolAddr))
                        col.Item().AlignCenter().Text(schoolAddr)
                            .FontSize(7.5f).FontColor(Colors.White);
                    var contact = new[] { schoolPhone, schoolEmail }
                        .Where(s => !string.IsNullOrEmpty(s));
                    if (contact.Any())
                        col.Item().AlignCenter().Text(string.Join("  •  ", contact))
                            .FontSize(7.5f).FontColor(Colors.White);
                });
                ...
                {
                    // Bill number row
                    col.Item().Background(lightBg).Padding(8).Row(row =>
                    {
                        row.RelativeItem().Column(c =>
                        {
                            c.Item().Text("PAYMENT RECEIPT").FontSize(7).FontColor(mutedText).Bold();
                            c.Item().Text(data.BillNumber).Bold().FontSize(13).FontColor(darkText);
                        });
                        row.RelativeItem().AlignRight().Column(c =>
                        {
                            c.Item().Text("DATE").FontSize(7).FontColor(mutedText).Bold();
                            c.Item().Text(data.BillDate.ToString("dd MMM yyyy")).Bold().FontSize(10);
                        });
                        row.AutoItem().AlignRight().PaddingLeft(8).Column(c =>
                        {
                            var statusColor = data.Status == "Paid" ? greenText
                                           : data.Status == "Partial" ? yellowText : red;
                            c.Item().Background(statusColor).Padding(4)
                                .Text(data.Status).Bold().FontSize(8).FontColor(Colors.White);
                        });
                    });
                    ...
                    // Student Details
                    col.Item().Text("STUDENT DETAILS").FontSize(7).Bold().FontColor(mutedText);
                    ...
                    col.Item().PaddingTop(4).Table(tbl =>
                    {
                        ...
                        {
                            tbl.Cell().PaddingBottom(3).Text(label).FontColor(mutedText).FontSize(8);
                            tbl.Cell().PaddingBottom(3).Text(val).Bold().FontSize(8);
                        }
                        Row("Full Name", data.StudentName);
                        Row("NIC Number", data.StudentNic);
                        Row("Phone", data.StudentPhone);
                    ...
                    // Payment Breakdown
                    col.Item().Text("PAYMENT BREAKDOWN").FontSize(7).Bold().FontColor(mutedText);
                    col.Item().LineHorizontal(0.5f);
                    col.Item().PaddingTop(4).Table(tbl =>
                    {
                        tbl.ColumnsDefinition(c => { c.RelativeColumn(); c.ConstantColumn(90); });

                        void AmtRow(string label, string val, string? color = null, bool bold = false)
                        {
                            tbl.Cell().PaddingBottom(3).Text(label)
                                .FontSize(8).FontColor(color ?? darkText);
                            var t = tbl.Cell().PaddingBottom(3).AlignRight().Text(val).FontSize(8);
                            if (bold) t.Bold();
                            if (color != null) t.FontColor(color);
                        }

                        AmtRow("Course Fee (Package Price)", $"Rs. {data.TotalAmount:N2}");
                        if (data.DiscountAmount > 0)
                            AmtRow("Discount", $"- Rs. {data.DiscountAmount:N2}", greenText);

                        tbl.Cell().ColumnSpan(2).LineHorizontal(0.5f).LineColor(red);

                        AmtRow("This Instalment (Net)",   $"Rs. {data.NetAmount:N2}",  red,       true);
                        AmtRow("Amount Paid",              $"Rs. {data.PaidAmount:N2}", greenText, true);

                        if (data.BalanceAmount > 0)
                            AmtRow("Balance Due",          $"Rs. {data.BalanceAmount:N2}", yellowText, true);
                    });

                    if (data.PaidAmount > 0)
                    {
                        col.Item().PaddingTop(8);
                        col.Item().Background(Colors.Green.Lighten4).Padding(8).Column(c =>
                        {
                            c.Item().Text($"✅  Paid via {data.PaymentMethod}")
                                .Bold().FontSize(8.5f).FontColor(Colors.Green.Darken2);
                            if (!string.IsNullOrEmpty(data.ReferenceNo))
                                c.Item().Text($"Reference: {data.ReferenceNo}").FontSize(7.5f);
                            if (!string.IsNullOrEmpty(data.Remarks))
                                c.Item().Text($"Note: {data.Remarks}").FontSize(7.5f);
                        });
                    }
                });

                // ── Footer ────────────────────────────────────────────────
                page.Footer().PaddingTop(8).BorderTop(0.5f).Column(col =>
                {
                    col.Item().AlignCenter().Text($"Thank you for choosing {schoolName}!")
                        .Bold().FontSize(9).FontColor(red);
                    if (data.BalanceAmount > 0)
                        col.Item().AlignCenter().Text($"Balance of Rs. {data.BalanceAmount:N2} is due. Please settle at your earliest convenience.")
                            .FontSize(7.5f).FontColor(yellowText);
                    col.Item().AlignCenter().Text($"Generated on {DateTime.Now:dd MMM yyyy HH:mm}  •  DSMS v2")
                        .FontSize(6.5f).FontColor(mutedText);
                });
            });
        });
